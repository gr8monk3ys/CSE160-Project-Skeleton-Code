...
socket_object_t createNewSocket(bool status, socket_t fd, socket_store_t store) {
...
implementation {
   ...
   command int LiveSocketList.checkIfPortIsListening(uint8_t port) {
      int i;
      for (i = 0; i <= socketCount; i++) {
         if (socketList[i].inUse &&
             socketList[i].store.state == SOCK_LISTEN &&
             socketList[i].store.sockAddr.srcPort == port) {
             return i;
         }
      }
   ...
   command int LiveSocketList.search(socket_addr_t *connection, socket_state status) {
      int i;

      for (i = 0; i <= socketCount; i++) {
           if (socketList[i].inUse &&
              socketList[i].store.state == status &&
              connection->port == socketList[i].store.sockAddr.srcPort &&
              connection->srcPort == socketList[i].store.sockAddr.port &&
